...
var siteLang = siteConfig.lang || "zh";
...
function presentationImage(index) {
    var images = [
        "image/user/01.jpg",
        "image/user/02.jpg"
    ];
...
function renderHero(profile, siteSetting) {
    ...
    $('#heroTitle').html('<span></span>' + escapeHtml(heroTitle));
...
function renderPublications(publications) {
    ...
    (publications || []).forEach(function(item, index) {
        var html = '<li>' + (index + 1) + '.' + escapeHtml(item.citation) + '</li>';
...
function renderPresentations(presentations) {
    ...
    if (!items.length) {
        ...
        imageHtml = '<div class="slh-userpic" style="background: url(' + presentationImage(0) + ') no-repeat center; background-size: cover;"></div>';
    } else {
        items.forEach(function(item, index) {
            ...
            slideHtml += '<div class="slhtesti-wrap"><div class="slh-innertesti"><h3 class="nameuser">' +
                ...
                '</ul></div></div>';
            imageHtml += '<div class="slh-userpic" style="background: url(' + presentationImage(index) + ') no-repeat center; background-size: cover;"></div>';
        });
    }
